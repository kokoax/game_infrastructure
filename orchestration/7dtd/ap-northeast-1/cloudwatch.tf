...
resource "aws_cloudwatch_event_rule" "notice_terminate_to_slack_lambda_event_rule" {
  name = "notice_terminate_to_slack_lambda_event_rule"
  description = "notice ec2 terminate to slack"
  event_pattern = <<PATTERN
{
  "source": [ "aws.ec2" ],
  "detail-type": [ "EC2 Instance State-change Notification" ],
...
resource "aws_cloudwatch_event_target" "notice_terminate_to_slack_lambda_event_target" {
  rule      = aws_cloudwatch_event_rule.notice_terminate_to_slack_lambda_event_rule.name
  target_id = "notice_terminate_to_slack_lambda"
  arn       = aws_lambda_function.notice_terminate_to_slack_lambda.arn
}

resource "aws_lambda_permission" "allow_cloudwatch_to_call_notice_terminate_to_slack" {
    statement_id  = "AllowExecutionFromCloudWatch"
    action        = "lambda:InvokeFunction"
    function_name = aws_lambda_function.notice_terminate_to_slack_lambda.function_name
    principal     = "events.amazonaws.com"
    source_arn    = aws_cloudwatch_event_rule.notice_terminate_to_slack_lambda_event_rule.arn
}
